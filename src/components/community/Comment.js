...
const Comment = () => {
    ...
    const commentEdit = (id) => {
        ...
        if (Object.keys(startEditComment).length) {
            ...
            if (editCommentValue !== originalCommentValue) {
                ...
                api.post("/edit/comment", { "content": editCommentValue, "commentId": id })
                    .then((response) => {
                        // 전송 성공 시 span 복원
                        if (originalSpan) {
                            commentEl.insertBefore(document.createElement("br"), commentEl.firstChild);
                            commentEl.insertBefore(originalSpan, commentEl.firstChild);
                        }
    ...
    const delComment = (id) => {
        console.log("삭제");
        api.post("/del/comment", { "commentId": id })
            .then((response) => {
                setCommentRe(prev => !prev);
            })
            .catch((error) => {
    ...
    return (
        <div className="comment-container">
            ...
            <div className="comment-content">
                {comments.map(parent => (
                    <div className={"del" + parent.commentId + " parent-comment"} key={parent.commentId}>
                                ...
                                {parent.userId === localStorage.getItem("userId") && (
                                    <>
                                        <button onClick={() => commentEdit(parent.commentId)}>
                                            {startEditComment[parent.commentId] ? "수정완료" : "답글수정"}
                                        </button>
                                        <button onClick={() => delComment(parent.commentId)}>답글삭제</button>
                                    </>
                                )}
                                {parent.childCount >= CHILD_OPEN_BUTTON_COUNT &&
                                    <button onClick={() => openChildComment(parent.commentId)}>
                                        {showChildComment[parent.commentId] ? `닫기` : `답글열기(${parent.childCount})`}
                                    </button>}
                                <button onClick={() => openChildInput(parent.commentId, parent.nickName)}>
                            ...
                            {showChildInput[parent.commentId] && (
                                <div className="child-comment-form">
                                    <div
                                        contentEditable="true"
                                        suppressContentEditableWarning={true}
                                        className={"child" + parent.commentId + " comment-input-div"}
                                        style={{ whiteSpace: "pre-wrap", wordBreak: "break-all" }}>
                                    </div>
                                    <button onClick={() => handlerChildSubmit(parent.commentId, parent.commentId)}>등록</button>
                                </div>
                            )}
                                        ...
                                        <div
                                            ...
                                            <div className="comment-body">
                                                ...
                                                <div
                                                    ...
                                                    className={"edit" + child.commentId + " comment-content-value"}
                                                    dangerouslySetInnerHTML={{ __html: child.content }}
                                                />
                                                <div className="comment-createAt">{child.time}</div>
                                                <div className="comment-button-div">
                                                    {child.userId === localStorage.getItem("userId") && (
                                                        <>
                                                            <button onClick={() => commentEdit(child.commentId)}>
                                                                {startEditComment[child.commentId] ? "수정완료" : "답글수정"}
                                                            </button>
                                                            <button onClick={() => delComment(child.commentId)}>답글삭제</button>
                                                        </>
                                                    )}
                                                    <button onClick={() => openChildInput(child.commentId, child.nickName)}>답글작성</button>
                                                </div>
                                                {showChildInput[child.commentId] && (
                                                    <div className="child-comment-form">
                                                        <div
                                                            contentEditable="true"
                                                            suppressContentEditableWarning={true}
                                                            className={"child" + child.commentId + " comment-input-div"}
                                                            style={{ whiteSpace: "pre-wrap", wordBreak: "break-all" }}>
                                                        </div>
                                                        <button onClick={() => handlerChildSubmit(parent.commentId, child.commentId)}>등록</button>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
