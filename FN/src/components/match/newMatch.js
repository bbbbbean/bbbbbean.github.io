...
const NewMatch = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "title" && value.length > 25) return;
    setIsWarn("");
        ...
        <form className="new-match-form" onSubmit={handleSubmit}>
          ...
          <div className="new-match-kategorie">
            ...
            <div>
              <div>
                <span
                  className={onKategorie == 1 ? "new-match-check" : ""}
                  value="1"
                  onClick={() => {
                    setOnKategorie(1);
                    setForm((prev) => ({ ...prev, kategorie: 1 }));
                  }}
                >
                  운동
                </span>
                <span
                  className={onKategorie == 2 ? "new-match-check" : ""}
                  value="2"
                  onClick={() => {
                    setOnKategorie(2);
                    setForm((prev) => ({ ...prev, kategorie: 2 }));
                  }}
                >
                  여행
                </span>
                <span
                  className={onKategorie == 3 ? "new-match-check" : ""}
                  value="3"
                  onClick={() => {
                    setOnKategorie(3);
                    setForm((prev) => ({ ...prev, kategorie: 3 }));
                  }}
                >
                  게임
                </span>
                <span
                  className={onKategorie == 4 ? "new-match-check" : ""}
                  value="4"
                  onClick={() => {
                    setOnKategorie(4);
                    setForm((prev) => ({ ...prev, kategorie: 4 }));
                  }}
                >
                  기타
                </span>
          ...
          <div>
            ...
            <input
              type="text"
              name="title"
              placeholder="25자까지 입력 가능합니다"
              value={form.title}
              onChange={handleChange}
            />
            {form.title.length >= 25 && (
              <span className="new-match-warning">
                25자까지 입력 가능합니다.
              </span>
            )}
          </div>
          ...
          <div
            className={
              isOnline
                ? "new-match-on new-match-defalt"
                : "new-match-off new-match-defalt"
            }
          >
            <label>위치</label>
            <div>
              <div>
                <span
                  className={isOnline ? "new-match-check" : ""}
                  onClick={() => {
                    setIsOnline(true);
                    setForm((prev) => ({ ...prev, location: "온라인" }));
                  }}
                >
                  온라인
                </span>
              ...
              <div
                style={isOnline ? { display: "none" } : { display: "block" }}
              >
          ...
          <div>
            ...
            />
            <p className="match-tag-help">공백으로 태그 입력 가능</p>
          </div>
          <div className="match-tag-wrap">
            {tags.map((tag, index) => (
          ...
          <div className="match-submit-btn-wrap">
            <button type="submit" className="match-submit-btn">매칭 등록</button>
          </div>
        </form>
      </div>
    </>
